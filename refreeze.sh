...
echo '[INFO][dhall-types-env][refreeze.sh] Refreezing...'
...
for depth in $(seq "${deepest_level}" -1 1) ; do
  while IFS= read -r -d '' package_dhall ; do
    echo "[INFO][dhall-types-env][refreeze.sh] Freezing: ${package_dhall} ..."
    dhall freeze --cache --all --inplace "${package_dhall}"
  done < <(find . -mindepth "${depth}" -maxdepth "${depth}" -name 'package.dhall' -print0)
done
echo '[INFO][dhall-types-env][refreeze.sh] Finished refreezing successfully!'
